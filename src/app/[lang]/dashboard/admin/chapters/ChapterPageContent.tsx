    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6 font-sans">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Courses Card */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-all duration-300 animate-slide-up">
            ...
            <div className="space-y-4">
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                ...
                <select
                  value={selectedCourse ?? ""}
                  onChange={(e) => setSelectedCourse(Number(e.target.value))}
                  className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white text-slate-700 font-medium"
                >
                  <option value="" className="text-slate-400">Choisir un cours</option>
                  {courses.map((c) => (
                    <option key={c.id} value={c.id} className="py-2">{c.title}</option>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-slate-600 mt-1 text-sm">
                  {selectedLevel 
                    ? `${chapters.length} chapitre${chapters.length !== 1 ? 's' : ''} disponible${chapters.length !== 1 ? 's' : ''}`
                    : "Sélectionnez un niveau pour voir les chapitres"
                  }
                </p>
              </div>
              ...
              {selectedLevel && (
                <button
                  className="group relative bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 overflow-hidden"
                  onClick={() => { setShowForm(true); setEditingChapter(null); }}
                >
            ...
            <div className="min-h-[200px]">
              {!selectedLevel ? (
                <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                  ...
                  <p className="text-center">Veuillez sélectionner un cours et un niveau</p>
                  <p className="text-center text-sm mt-2">pour afficher les chapitres</p>
                </div>
              ) : loading ? (
                <div className="flex items-center justify-center py-12">
                  <div className="animate-pulse flex flex-col items-center space-y-4">
                    <div className="w-10 h-10 bg-gradient-to-r from-blue-200 to-indigo-200 rounded-full animate-spin"></div>
                    <p className="text-slate-600 text-sm font-medium">Chargement des chapitres...</p>
                  </div>
                </div>
              ) : (
                <ChapterList
